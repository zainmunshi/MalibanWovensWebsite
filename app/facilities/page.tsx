...
interface Facility {
  ...
  highlights: string[]
  motif: 'corporate' | 'manufacturing' | 'sewing' | 'carton'
  px: number
  py: number
  color: string
}
...
function SriLankaMap({ active, onSelect }: { active: string | null; onSelect: (id: string) => void }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', maxWidth: 380 }}>
      <svg viewBox="0 0 380 633" style={{ width: '100%', height: 'auto', overflow: 'visible' }}>
        <defs>
          <filter id="pinGlow">
        ...
        <path d={SL_PATH} fill="#0f2a1a" stroke="rgba(26,92,56,0.6)" strokeWidth="1.5" />
        <path d={SL_PATH} fill="url(#gridPat)" opacity="0.15" />
        ...
        {FACILITIES.map((f, i) =>
          FACILITIES.slice(i + 1).map((f2) => {
            ...
            return (
              <line key={`${f.id}-${f2.id}`} x1={f.px} y1={f.py} x2={f2.px} y2={f2.py}
                stroke="rgba(74,222,128,0.08)" strokeWidth="0.8" strokeDasharray="3 4" />
            )
          })
        )}
        ...
        {FACILITIES.map((f) => {
          ...
          return (
            <g key={f.id} transform={`translate(${f.px}, ${f.py})`}
...
/* ── Facility Motif SVG ────────────────────────────────────────────── */
function FacilityMotif({ motif, color }: { motif: Facility['motif']; color: string }) {
  if (motif === 'corporate') {
    return (
      <svg viewBox="0 0 240 160" style={{ width: '100%', height: '100%' }} preserveAspectRatio="xMidYMid meet">
        <defs>
        ...
        <circle cx="120" cy="80" r="60" fill="url(#ho-glow)" />
        {[20, 35, 50].map((r, i) => (
          <circle key={i} cx="120" cy="80" r={r}
            fill="none" stroke={color}
            strokeWidth={i === 1 ? 1 : 0.6}
            opacity={0.3 + i * 0.15}
            strokeDasharray={i === 0 ? "0" : i === 1 ? "2 3" : "1 4"} />
        ))}
        {[0, 60, 120, 180, 240, 300].map((deg) => {
          const rad = (deg * Math.PI) / 180
          const x = 120 + Math.cos(rad) * 65
          const y = 80 + Math.sin(rad) * 65
          return (
            <g key={deg}>
        ...
        <circle cx="120" cy="80" r="6" fill={color} />
        <circle cx="120" cy="80" r="3" fill="#040d1a" />
      </svg>
    )
  }

  if (motif === 'manufacturing') {
    return (
      <svg viewBox="0 0 240 160" style={{ width: '100%', height: '100%' }} preserveAspectRatio="xMidYMid meet">
        <defs>
          <linearGradient id="mfg-fade" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0%" stopColor={color} stopOpacity="0" />
            <stop offset="50%" stopColor={color} stopOpacity="1" />
            <stop offset="100%" stopColor={color} stopOpacity="0" />
          </linearGradient>
        </defs>
        <line x1="20" y1="120" x2="220" y2="120" stroke={color} strokeWidth="1" opacity="0.4" />
        <line x1="20" y1="124" x2="220" y2="124" stroke={color} strokeWidth="0.5" opacity="0.2" />
        {[
          { x: 30, h: 50 }, { x: 45, h: 65 }, { x: 60, h: 40 },
          { x: 75, h: 75 }, { x: 90, h: 55 }, { x: 105, h: 70 },
          { x: 120, h: 85 }, { x: 135, h: 60 }, { x: 150, h: 78 },
          { x: 165, h: 45 }, { x: 180, h: 68 }, { x: 195, h: 55 }, { x: 210, h: 72 },
        ].map((p, i) => (
          <g key={i}>
            <rect x={p.x - 3} y={120 - p.h} width="6" height={p.h}
              fill={color} opacity={0.15 + (i % 3) * 0.15} />
            <rect x={p.x - 3} y={120 - p.h} width="6" height="2" fill={color} />
          </g>
        ))}
        <line x1="20" y1="35" x2="220" y2="35" stroke="url(#mfg-fade)" strokeWidth="0.8" />
        <path d="M 20 35 L 20 30 M 220 35 L 220 30" stroke={color} strokeWidth="1" opacity="0.4" />
      </svg>
    )
  }
  ...
  if (motif === 'sewing') {
    return (
      <svg viewBox="0 0 240 160" style={{ width: '100%', height: '100%' }} preserveAspectRatio="xMidYMid meet">
        <defs>
          <pattern id="stitch-grid" width="12" height="12" patternUnits="userSpaceOnUse">
            <circle cx="6" cy="6" r="0.5" fill={color} opacity="0.2" />
          </pattern>
        </defs>
        <rect width="240" height="160" fill="url(#stitch-grid)" />
        <path d="M 20 80 Q 60 30, 100 80 T 180 80 T 220 80"
          fill="none" stroke={color} strokeWidth="1.2" strokeDasharray="6 4"
          opacity="0.7" />
        {[
          { x: 40, y: 55, r: 90 }, { x: 80, y: 55, r: -90 },
          { x: 120, y: 80, r: 0 }, { x: 160, y: 105, r: 90 },
          { x: 200, y: 80, r: 0 },
        ].map((n, i) => (
          <g key={i} transform={`translate(${n.x},${n.y}) rotate(${n.r})`}>
            <line x1="-4" y1="0" x2="4" y2="0" stroke={color} strokeWidth="1.5" />
            <circle cx="-4" cy="0" r="1.2" fill={color} />
          </g>
        ))}
        <g opacity="0.4">
          {[40, 80, 120, 160, 200].map((x, i) => (
            <line key={i} x1={x} y1="135" x2={x} y2="145" stroke={color} strokeWidth="0.8" />
          ))}
        </g>
      </svg>
    )
  ...
  return (
    <svg viewBox="0 0 240 160" style={{ width: '100%', height: '100%' }} preserveAspectRatio="xMidYMid meet">
      ...
      {[
        ...
        const w = 28 * b.s, h = 22 * b.s, d = 12 * b.s
        return (
          <g key={i}>
            <path d={`M ${b.x} ${b.y} L ${b.x + w} ${b.y} L ${b.x + w + d} ${b.y - d} L ${b.x + d} ${b.y - d} Z`}
              fill="url(#ct-face)" stroke={color} strokeWidth="0.8" />
            <path d={`M ${b.x} ${b.y} L ${b.x + w} ${b.y} L ${b.x + w} ${b.y + h} L ${b.x} ${b.y + h} Z`}
              fill={color} fillOpacity="0.15" stroke={color} strokeWidth="0.8" />
            <path d={`M ${b.x + w} ${b.y} L ${b.x + w + d} ${b.y - d} L ${b.x + w + d} ${b.y + h - d} L ${b.x + w} ${b.y + h} Z`}
              fill={color} fillOpacity="0.08" stroke={color} strokeWidth="0.8" />
            <line x1={b.x + w/2} y1={b.y - d} x2={b.x + w/2} y2={b.y + h}
              stroke={color} strokeWidth="0.5" strokeDasharray="2 2" opacity="0.5" />
          </g>
        )
      })}
      <line x1="40" y1="135" x2="200" y2="135" stroke={color} strokeWidth="0.5" opacity="0.3" />
    </svg>
  )
}

/* ── Facility Detail Modal ─────────────────────────────────────────── */
/* Full-screen overlay — fits everything in viewport, no scroll needed   */
    ...
    >
      <div
        ...
        style={{
          ...
          background: 'linear-gradient(140deg, #0a1929 0%, #040d1a 50%, #050e1c 100%)',
          border: `1px solid ${facility.color}33`,
          borderRadius: 20,
          overflow: 'hidden',
          boxShadow: `0 40px 100px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.04), 0 0 80px ${facility.color}22`,
      ...
      >
        ...
        <div style={{
          ...
          background: `radial-gradient(circle, ${facility.color}33 0%, transparent 60%)`,
          pointerEvents: 'none',
        }} />
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24 }}>
                <div style={{
                  ...
                  border: `1.5px solid ${facility.color}66`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 16, fontWeight: 800,
                  color: facility.color,
                  fontFamily: 'monospace', letterSpacing: '0.05em',
                  boxShadow: `0 0 24px ${facility.color}33`,
                }}>{facility.code}</div>
                <div style={{
                  ...
                  <div style={{ width: 6, height: 6, borderRadius: '50%', background: facility.color, boxShadow: `0 0 8px ${facility.color}` }} />
                  <span style={{
                    fontSize: 11, fontWeight: 700, letterSpacing: '0.15em',
                    color: facility.color, fontFamily: 'monospace', textTransform: 'uppercase',
                  }}>{facility.type}</span>
                </div>
              </div>
            ...
            <div>
              <div style={{
                ...
                <div style={{ width: 18, height: 1, background: facility.color }} />
                Capabilities
              </div>
              <div style={{
                display: 'grid',
                ...
                {facility.highlights.map((h, i) => (
                  <div key={i} style={{
                    ...
                    <div style={{
                      ...
                      background: `${facility.color}1a`,
                      border: `1px solid ${facility.color}55`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      flexShrink: 0,
                    }}>
                      <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                        <path d="M2 5l2 2L8 3" stroke={facility.color} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </div>
                    <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.75)', lineHeight: 1.4 }}>{h}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{
            ...
            {/* Top — large motif panel */}
            <div style={{
              position: 'relative',
              flex: 1,
              minHeight: 280,
              background: `linear-gradient(135deg, ${facility.color}14 0%, rgba(255,255,255,0.01) 100%)`,
              ...
              <div style={{ width: '100%', maxWidth: 380, position: 'relative', zIndex: 2 }}>
                <FacilityMotif motif={facility.motif} color={facility.color} />
              </div>

              {/* Corner crop marks */}
              <div style={{ position: 'absolute', top: 16, left: 16, width: 14, height: 14, borderTop: `1.5px solid ${facility.color}88`, borderLeft: `1.5px solid ${facility.color}88` }} />
              <div style={{ position: 'absolute', top: 16, right: 16, width: 14, height: 14, borderTop: `1.5px solid ${facility.color}88`, borderRight: `1.5px solid ${facility.color}88` }} />
              <div style={{ position: 'absolute', bottom: 16, left: 16, width: 14, height: 14, borderBottom: `1.5px solid ${facility.color}88`, borderLeft: `1.5px solid ${facility.color}88` }} />
              <div style={{ position: 'absolute', bottom: 16, right: 16, width: 14, height: 14, borderBottom: `1.5px solid ${facility.color}88`, borderRight: `1.5px solid ${facility.color}88` }} />

              {/* Bottom-left tag */}
              <div style={{
                position: 'absolute',
                bottom: 24, left: 24, zIndex: 3,
                fontSize: 10, fontFamily: 'monospace',
                letterSpacing: '0.2em',
                color: `${facility.color}aa`,
                textTransform: 'uppercase',
              }}>// {facility.code} · NODE</div>
            ...
            <div style={{
              display: 'grid',
              gridTemplateColumns: `repeat(${facility.stats.length}, 1fr)`,
              ...
              {facility.stats.map((s, i) => (
                <div key={i} style={{
                  padding: '24px 18px',
                  textAlign: 'center',
                  borderRight: i < facility.stats.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none',
                  position: 'relative',
                }}>
                  <div style={{
        ...
        <div style={{
          ...
          <span>MALIBAN WOVENS · OPERATIONAL NODE</span>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 40px' }}>
          ...
          <div style={{
            ...
            {/* Facility list — modal opens as overlay, list always visible */}
            <div className="reveal">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <div style={{ fontSize: 11, fontFamily: 'monospace', letterSpacing: '0.12em', color: 'rgba(255,255,255,0.25)', textTransform: 'uppercase', marginBottom: 16 }}>
                  — Select a pin or click a facility to explore
                </div>
                {FACILITIES.map((f) => (
                  <div
                    key={f.id}
        ...
        {activeFacility && (
          <FacilityModal facility={activeFacility} onClose={() => setActivePin(null)} />
        )}
      </section>
